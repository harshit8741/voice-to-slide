...
const processAudioFile = async (
  ...
  try {
    ...
    const transcription = transcriptionResult.trim();

    if (transcription.length < 10) {
      throw new Error(
        "Transcription too short or empty. Please provide a longer audio recording."
      );
    }

    // Generate slides from transcription
    const presentation =
      await slideGeneratorService.generateSlidesFromTranscription(
        transcription,
        userId,
        title
      );
  ...
  async (req, res): Promise<void> => {
    try {
      ...
      if (!req.file) {
        res
          .status(400)
          .json({ error: "No audio file or YouTube URL provided" });
        return;
      }
